...
The build process for `docker-cli`, `dockerd`, and `containerd` has been updated in the `termux-root-packages` repository. The original patch files are no longer available at the links provided. The patching and build logic can be found in the `build.sh` script for the `docker` and `containerd` packages.

### 4.2.1. dockerd patch

- [cmd/dockerd/daemon.go](https://raw.githubusercontent.com/termux/termux-root-packages/master/packages/docker/daemon.go.patch)

### 4.2.2. containerd patches

- [runtime/v1/linux/bundle.go](https://raw.githubusercontent.com/termux/termux-root-packages/master/packages/containerd/bundle.go.patch.txt)
- [runtime/v2/shim/util_unix.go](https://raw.githubusercontent.com/termux/termux-root-packages/master/packages/containerd/util_unix.go.patch.txt)
- [Makefile](https://raw.githubusercontent.com/termux/termux-root-packages/master/packages/containerd/Makefile.patch)
- [platforms/database.go](https://raw.githubusercontent.com/termux/termux-root-packages/master/packages/containerd/database.go.patch.txt)
- [vendor/github.com/cpuguy83/go-md2man/v2/md2man.go](https://raw.githubusercontent.com/termux/termux-root-packages/master/packages/containerd/md2man.go.patch.txt)
...
When attempting to build the kernel as described in this guide, several issues were encountered. These issues and their solutions are documented below to assist others who may encounter them.

### 6.1.1. Build Environment

The build process is sensitive to the toolchain and environment configuration. The original `build.sh` script in the repository is designed to be run in a specific environment, and may not work out of the box. It is recommended to use a Docker container with the necessary dependencies installed.

### 6.1.2. Cross-Compiler Toolchain

The kernel build requires both 64-bit and 32-bit cross-compilers. The following packages are required:

*   `gcc-aarch64-linux-gnu`
*   `gcc-arm-linux-gnueabihf`

These must be installed and in the system's `PATH`.

### 6.1.3. Link Time Optimization (LTO)

The kernel configuration enables Link Time Optimization (`CONFIG_LTO_CLANG`), which can cause issues with some versions of the `clang` compiler. If you encounter errors related to `regalloc`, it is recommended to disable LTO by adding the following to your build script:

```bash
scripts/config --file out/.config --disable CONFIG_LTO_CLANG
```

### 6.1.4. 32-bit Compatibility

The kernel build process attempts to build a 32-bit compatible vDSO, which requires the `CROSS_COMPILE_COMPAT` variable to be set. If you encounter an error related to this, ensure that the `gcc-arm-linux-gnueabihf` package is installed and that `CROSS_COMPILE_COMPAT` is set to `arm-linux-gnueabihf-`.